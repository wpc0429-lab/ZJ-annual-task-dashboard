...
const sheet = workbook.worksheets.getItem("应用建设");
const rows = sheet.getRange("A3:K86").values;
const styleByRow = new Map();
for (const range of ["C3:C30", "C31:C60", "C61:C86"]) {
  ...
  for (const line of styleInspection.ndjson.split("\n")) {
    if (!line.trim() || line.includes('"kind":"notice"')) continue;
    const record = JSON.parse(line);
    const rowNumber = Number(record.for?.match(/C(\d+)/)?.[1]);
    if (!rowNumber) continue;
    const fillColor = record.style?.fill?.color?.value ?? null;
    const fillTheme = record.style?.fill?.color?.value ?? null;
    styleByRow.set(rowNumber, {
      punch: fillTheme === "theme:9",
      yellow: fillColor === "FFFF00",
      blue: fillColor === "00B0F0",
    });
  }
}
...
rows.forEach((row, index) => {
  ...
  const style = styleByRow.get(excelRow) ?? {};
  group.punch ||= Boolean(style.punch);
  group.yellow ||= Boolean(style.yellow);
  group.blue ||= Boolean(style.blue);
...
const scenarios = [...groups.values()].map((group, index) => {
  ...
  // 确定 priority 标签
  let priority = null;
  if (mappedKind === "punch") priority = "punch";
  else if (mappedKind === "pending") priority = "blue";
  else if (group.yellow) priority = "yellow";
  else if (group.blue) priority = "blue";
